...
voidand (int rd, int rs, int rt, int format) {
	Registers[rd] = Registers[rs] & Registers[rt];
	PC = PC + format + 1;
}
...
voidxor (int rd, int rs, int rt, int format) {
	Registers[rd] = Registers[rs] ^ Registers[rt];
	PC = PC + format + 1;
}
...
void UpdateAndCheckDisk() {
	...
	if (IORegisters[17] == 0)  // diskstatus=0 (free to receive new command)
	{
		...
		if (IORegisters[14] == 2)  // diskcmd=2 (write sector)
		{
			...
			{
				Memory_Disk[IORegisters[15]][i] = Memory[IORegisters[16] + i];
...
void WriteToTrace(unsigned long long inst)
{
	fprintf(trace_file, "%03X %05X ", PC, inst);
