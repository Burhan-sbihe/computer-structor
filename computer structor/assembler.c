...
#include <stdlib.h>
#include <string.h>
// a line can have 500 chars according to instructions
#define MAX_LINE 500
...
label* createLabelList(FILE* asembl) {
	...
	int rowIndex = 0, k, j, option, counter = 0, check = 0;
	...
	while (!feof(asembl)) {
		...
		{
			if ((strstr(line, "$imm")) < (strstr(line, "#"))) counter = counter + 2; //check if $imm is before # 
		}
		else if (option == 0) counter++;
...
char* readrdst(char line[MAX_LINE], char* rdst, int* k) {
	int j = 0; // Read rd
	while (line[*k] != ',')
	{
...
char* readimmd(char line[MAX_LINE], char* imm, int* k) {
	// go to immediate
...
void PrintDataToFile(Memory* mem, FILE* memin)
{
	...
	int i = 0, num = 0; int flag = 0; int check = 0; int checkk = 0; char immediet[6] = { '$', 'i', 'm', 'm', '\0' };
	while (mem->head != NULL && i <= mem->last) {
		...
		else if (!flag && check == 0 && checkk == 0 && currentLine != NULL) // if there is nothing print a zero
			fprintf(memin, "00");
		if (currentLine == NULL) fprintf(memin, "0"); // Printing Rd
		else if (check == 0)print_rd_rs_rt(currentLine->rd, memin, &num);
		if (currentLine == NULL) fprintf(memin, "0"); 		// Printing Rs
		else if (check == 0) print_rd_rs_rt(currentLine->rs, memin, &num);
		if (currentLine == NULL) fprintf(memin, "0"); 		// Printing Rt
		else if (check == 0) print_rd_rs_rt(currentLine->rt, memin, &num);
		// a check wheter to print the immediate and skip .word lines
		if (currentLine != NULL && check == 0) {
			if ((strcmp(currentLine->rs, immediet) == 0) || (strcmp(currentLine->rd, immediet) == 0) || (strcmp(currentLine->rt, immediet) == 0)) {
				if ((currentLine->imm[0] == '0') && ((currentLine->imm[1] == 'x') || (currentLine->imm[1] == 'X')))
					num = strtol(currentLine->imm, NULL, 16);
				else  //Imiddiate is decimal
					num = atoi(currentLine->imm);
...
int main(int argc, char* argv[]) {
	...
	FILE* asembl = fopen("C:\\Users\\Abbas Admin\\source\\repos\\myproject\\myproject\\fib.txt", "r");
	...
	asembl = fopen("C:\\Users\\Abbas Admin\\source\\repos\\myproject\\myproject\\fib.txt", "r");
	...
	// Write Data to file
	FILE* memin = fopen("C:\\Users\\Abbas Admin\\source\\repos\\myproject\\myproject\\memin.txt", "w");
	if (memin == NULL)
		exit(1);
